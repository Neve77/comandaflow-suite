...
export default function BillingPanel({ summary, charges, loading, canWrite, onCreate, onEdit, onPay, onCancel, onHistory }) {
  const cards = [
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canWrite && <button className="btn-primary bg-blue-600 hover:bg-blue-700" onClick={() => onCreate(null)}><Plus size={17} />Nova cobrança</button>}
      </div>
          ...
          <table className="w-full min-w-[850px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {charges.map((charge) => <tr key={charge.id} className="hover:bg-slate-50/70">
                <td className="px-5 py-4 font-bold text-slate-900">{charge.subscriber.businessName}</td>
                <td className="px-5 py-4 text-slate-600">{charge.description || 'Mensalidade'}</td>
                <td className="px-5 py-4 font-extrabold text-slate-900">{formatMoney(charge.amount)}</td>
                <td className="px-5 py-4"><p className="font-semibold text-slate-700">{formatDate(charge.dueDate)}</p>{charge.paidAt && <p className="text-xs text-emerald-700">Pago em {formatDate(charge.paidAt)}</p>}</td>
                <td className="px-5 py-4"><span className={`status-chip ${statusStyle[charge.status]}`}>{charge.status}</span>{charge.paymentMethod && <p className="mt-1 text-xs text-slate-500">{charge.paymentMethod}</p>}</td>
                <td className="px-5 py-4"><div className="flex justify-end gap-2">
                  <button className="btn-secondary px-3" title="Histórico" onClick={() => onHistory(charge)}><History size={15} /></button>
                  {canWrite && ['pendente', 'vencida'].includes(charge.status) && <><button className="btn-secondary px-3" title="Editar" onClick={() => onEdit(charge)}><Pencil size={15} /></button><button className="btn-primary bg-emerald-600 px-3 hover:bg-emerald-700" onClick={() => onPay(charge)}><ReceiptText size={15} />Receber</button><button className="btn-secondary px-3 text-rose-700" onClick={() => onCancel(charge)}>Cancelar</button></>}
                </div></td>
              </tr>)}
              {!loading && charges.length === 0 && <tr><td colSpan="6" className="px-5 py-12 text-center text-slate-500">Nenhuma cobrança cadastrada.</td></tr>}
              {loading && <tr><td colSpan="6" className="px-5 py-12 text-center text-slate-500">Carregando cobranças...</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
